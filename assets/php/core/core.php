...
class core {
  ...
  public static function partials($content, $settings){
    ...
    while (strpos($content,'{{partial: ') !== false) {
	  ...
	  if(in_array($partial, array("fxpro","fxpro-widget"))) {

		  require_once ROOT_DIR . 'assets/ip2l/IP2Location.php';

		  $db = new \IP2Location\Database(ROOT_DIR . 'assets/ip2l/IP-COUNTRY.BIN', \IP2Location\Database::MEMORY_CACHE);

		  $cc = $db->lookup($_SERVER['REMOTE_ADDR'], \IP2Location\Database::COUNTRY);

		  if(in_array($cc['countryCode'], array("BE","CA","FR","IR","IL","JP","KR","KP","NZ","TR","AZ","US"))) {
