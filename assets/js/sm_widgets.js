...
(function($) {
  ...
  var yqlNewsHeadlineUrl = 'http://finance.yahoo.com/rss/headline?s=';
  ...
  window.smBuildWidgets = buildWidgets;
  ...
  function buildWidgets() {
    ...
    while (historicalPerformanceSymbols.length) {
      log('Retrieve historical performance for tickers: '+historicalPerformanceSymbols.join(','));
      var histCondiditons = [];
      var liveSymbols = [];
      //slicing symbols into chunks
      $.each(historicalPerformanceSymbols.splice(0,30), function(k, historicalPerformance) {
...
function smFormatNumber(number) {
  var precision = typeof arguments[1]!='undefined' && typeof arguments[1][0]!='undefined' ? arguments[1][0] : 2;
  var prefix = typeof arguments[1]!='undefined' && typeof arguments[1][1]!='undefined' ? arguments[1][1] : '';
  var suffix = typeof arguments[1]!='undefined' && typeof arguments[1][2]!='undefined' ? arguments[1][2] : '';
  number = parseFloat(number);
  number = precision>1 ? Math.round(number * Math.pow(10,precision))/Math.pow(10,precision) : Math.round(number);
  return number>=0 ? prefix+(number.toLocaleString('en-US'))+suffix : '-'+prefix+Math.abs(number.toLocaleString('en-US'))+suffix;
}
